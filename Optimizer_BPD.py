...
def Optimizer_with_BPD(n, m, XORs, NLs, NOTs, log_filename='log', H=999):
    ...
    while (max(Dist) > 0):
        ...
        if is_there_g >= 0:
            Sname.append(f'g[{i}]')
            S.append(1 << (n+i))
            D.append(max(min([D[_] for _ in range(len(S)) if (Y[2*i] == S[_]) and (D[_] <= HY[2*i])]),
                     min([D[_] for _ in range(len(S)) if (Y[2*i+1] == S[_]) and (D[_] <= HY[2*i+1])])) + 1)
            pre_emp = True
            ...
            for w in WD:
                ...
                for i in range(len(Y)):
                    if Dist[i] == 0:
                        continue
                    test_Dist[i] = update_distance_for_new_pair(
                        S+[w], D+[WD[w]], Y[i], HY[i], Dist[i])
...
def update_S_XORs_for_new_pair(w, d):
    ...
    S_XORs_2PD_for_new_pair[2] = {
        w ^ s: (1 << d)+S_XORs_2PD[1][s] for s in S_XORs_2PD[1]}
    S_XORs_2PD_for_new_pair[3] = {
        w ^ s: (1 << d)+S_XORs_2PD[2][s] for s in S_XORs_2PD[2]}
    S_XORs_2PD_for_new_pair[4] = {
        w ^ s: (1 << d)+S_XORs_2PD[3][s] for s in S_XORs_2PD[3]}
...
def limit_depths(n, m, k, Y, H):
    ...
    for i in range(k):
        r = Y[2*i]
        D[r] = math.ceil(math.log2(sum([1 << D[x] for x in X[Y.index(r)]])))
        r = Y[2*i+1]
        D[r] = math.ceil(math.log2(sum([1 << D[x] for x in X[Y.index(r)]])))
        g = 1 << (n+i)
    ...
    for i in range(m):
        y = Y[2*k+i]
        D[y] = math.ceil(math.log2(sum([1 << D[x] for x in X[Y.index(y)]])))
...
def check_max_depth(n, m, k, D_based, Y, X):
    ...
    for i in range(k):
        ...
        if r not in D:
            D[r] = math.ceil(
                math.log2(min([sum([1 << D[x] for x in XOR]) for XOR in X[Y.index(r)]])))
        r = Y[2*i+1]
        if r not in D:
            D[r] = math.ceil(
                math.log2(min([sum([1 << D[x] for x in XOR]) for XOR in X[Y.index(r)]])))
    ...
    for i in range(m):
        ...
        if y not in D:
            D[y] = math.ceil(
                math.log2(min([sum([1 << D[x] for x in XOR]) for XOR in X[Y.index(y)]])))
...
def make_Circuit(n, m, k, S, D, Y, NLs, NOTs):
    ...
    Circuit = []
    r_not_fin = [i for i in range(2*k)]
    y_not_fin = [i for i in range(m)]
    for i in range(len(S)):
        b = S[i]
        if 'x' in B_match[b]:
            pass
